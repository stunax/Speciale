...
weights_path = '/home/dpj482/data/model_weights/%s_%i_%i_weights.h5'
...
def make_autoencoder_model(args):
    ...
    conv1 = Conv2D(32, (5, 5), activation='relu', padding='same')(drop1)
    pool1 = MaxPooling2D(pool_size=(2, 2))(conv1)
    drop2 = Dropout(args.dropout)(pool1)
    conv2 = Conv2D(64, (5, 5), activation='relu', padding='same')(drop2)
    pool2 = MaxPooling2D(pool_size=(2, 2))(conv2)
    drop3 = Dropout(args.dropout)(pool2)
    conv3 = Conv2D(128, (3, 3), activation='relu', padding='same')(drop3)
    dense0 = Flatten()(conv3)
    drop4 = Dropout(args.dropout)(dense0)
    dense1 = Dense(1024, activation='relu')(drop4)
    encoded = Dense(encoded_size, activation='relu')(dense1)
    # decoder
    dense2 = Dense(1024)(encoded)
    reshape1 = Reshape((4, 4, 64))(dense2)
    conv4 = Conv2D(128, (3, 3), activation='relu', padding='same')(reshape1)
    up1 = UpSampling2D((2, 2))(conv4)
    conv5 = Conv2D(64, (5, 5), activation='relu', padding='same')(up1)
    up2 = UpSampling2D((2, 2))(conv5)
    conv6 = Conv2D(32, (5, 5), activation='relu', padding='same')(up2)
    up3 = UpSampling2D((2, 2))(conv6)
    decoded = Conv2D(patch_size[-1], (5, 5),
                     activation=None, padding='same')(up3)
...
def train(model, data_model, h5s, args, n, callbacks, input_target):
    ...
    X_test_batcher = data_model.as_batcher(
        X_test, config.batch_size, 9999999, input_target=True)
...
def train_encoder(model, args):
    ae_weights_path = weights_path % ("ae", args.normalize, args.median_time)

    if use_saved_weights and os.path.isfile(ae_weights_path):
...
def train_predictor(model, args):
    pred_weights_path = weights_path % (
        "pred", args.normalize, args.median_time)

    # if False:  # use_saved_weights and os.path.isfile(pred_weights_path):
    if use_saved_weights and os.path.isfile(pred_weights_path):
        print("loading weights")
        model.load_weights(pred_weights_path)
        return

    checkpoint = callbacks.ModelCheckpoint(
        weights_path % ("pred", args.normalize, args.median_time),
        monitor='val_loss', verbose=1,
        save_best_only=True, mode='min')
    tb = callbacks.TensorBoard(
